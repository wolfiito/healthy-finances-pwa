...
function App() {
  ...
  const content = {
    ...
    wallet: <Wallet data={data} open={setDialog} remove={remove} />,
...
function Auth({ onLogin }) {
  ...
  const submit = async (event) => {
    ...
    try {
      if (register) await api.post('/api/auth/register', { username, password })
      const response = await api.post('/api/auth/login', { username, password }); onLogin(response.data.token)
    } catch (e) { setError(e.message) }
  }
  return <div className="auth-page"><div className="auth-orb orb-one" /><div className="auth-orb orb-two" />
    <section className="auth-card"><Brand /><p className="eyebrow">FINANZAS PERSONALES</p><h1>Haz que tu dinero se sienta bien.</h1>
      <p className="subcopy">Una vista suave, clara y poderosa para cuidar cada peso.</p>
      <form onSubmit={submit} className="form-stack"><Field label="Usuario" value={username} onChange={(e) => setUsername(e.target.value)} placeholder="Tu usuario" required />
        ...
        <button className="button primary wide">{register ? 'Crear mi espacio' : 'Entrar a mis finanzas'} <FiArrowUpRight /></button></form>
      {error && <p className="form-error">{error}</p>}<button className="link-button" onClick={() => setRegister(!register)}>{register ? 'Ya tengo una cuenta' : 'Soy nueva por aquí'}</button>
    </section></div>
}
...
function Wallet({ data, open, remove }) { const [tab, setTab] = useState('accounts'); return <section className="page"><div className="section-line"><div><h2>Todo bajo control</h2><p>Tu dinero, tarjetas y deudas en un solo lugar.</p></div><button className="button primary" onClick={() => open({ type: tab === 'accounts' ? 'account' : 'debt' })}><FiPlus /> {tab === 'accounts' ? 'Cuenta' : 'Deuda'}</button></div><div className="segmented"><button className={tab === 'accounts' ? 'active' : ''} onClick={() => setTab('accounts')}>Cuentas</button><button className={tab === 'debts' ? 'active' : ''} onClick={() => setTab('debts')}>Deudas</button></div>{tab === 'accounts' ? <div className="card-grid">{data.accounts.length ? data.accounts.map((item) => <article key={accountId(item)} className="account-card"><span className="account-icon"><FiCreditCard /></span><p>{accountNames[item.account_type || item.type] || item.account_type}</p><h3>{item.account_name || item.name}</h3><strong>{money.format(Number(item.current_balance || 0))}</strong><div><button onClick={() => open({ type: 'account', item })}><FiEdit3 /> Editar</button><button onClick={() => remove(`/api/accounts/${accountId(item)}`, 'esta cuenta')}><FiTrash2 /></button></div></article>) : <Empty text="Agrega una cuenta de efectivo, débito o crédito." />}</div> : <Surface title="Mis deudas"><div className="debt-list">{data.debts.length ? data.debts.map((item) => { const original = Number(item.original_amount); const remaining = Number(item.remaining_amount); const percent = original ? Math.min(100, (original - remaining) / original * 100) : 0; return <article key={item.debt_id} className="debt-card"><div><span className="round expense"><FiCreditCard /></span><div><h3>{item.debt_name}</h3><small>Pago mensual {money.format(Number(item.monthly_payment_amount))}</small></div></div><strong>{money.format(remaining)}</strong><i><em style={{ width: `${percent}%` }} /></i><footer><span>{percent.toFixed(0)}% liquidado</span><div><button onClick={() => open({ type: 'debt', item })}><FiEdit3 /></button><button onClick={() => remove(`/api/debts/${item.debt_id}`, 'esta deuda')}><FiTrash2 /></button></div></footer></article> }) : <Empty text="Aquí aparecerán las deudas que registres." />}</div></Surface>}</section> }
