...
import { BarChart3, CircleDollarSign, Home as HomeIcon, Landmark, Plus, ReceiptText, Scale, Settings as SettingsIcon, WalletCards, X } from 'lucide-react'
...
function Home({ data, onNavigate }) {
  const balance = Number(data.balance?.current_balance || 0); const debtTotal = data.debts.reduce((sum, debt) => sum + Number(debt.remaining_amount), 0)
...
function BottomBar({ active, onNavigate, quickOpen, onQuickToggle, onQuickAction }) {
  const items = [{ id: 'home', Icon: HomeIcon, label: 'Inicio' }, { id: 'accounts', Icon: WalletCards, label: 'Cuentas' }, { id: 'projection', Icon: BarChart3, label: 'Proyección' }, { id: 'settings', Icon: SettingsIcon, label: 'Ajustes' }]
  ...
  return <>{quickOpen && <button className="quick-backdrop" aria-label="Cerrar menú" onClick={onQuickToggle} />}<div className={`quick-actions ${quickOpen ? 'is-open' : ''}`}>{actions.map((action, index) => <button key={action.id} className={`quick-action action-${index + 1}`} onClick={() => onQuickAction(action.id)}><span><action.Icon size={27} strokeWidth={2.2} /></span><small>{action.label}</small></button>)}</div><nav className="bottom-bar" aria-label="Navegación principal"><div className="bar-items"><div className="nav-group">{items.slice(0, 2).map((item) => <NavItem key={item.id} item={item} active={active === item.id} onClick={() => onNavigate(item.id)} />)}</div><div className="quick-wrap"><button className={`quick-button ${quickOpen ? 'open' : ''}`} onClick={onQuickToggle} aria-label="Agregar">{quickOpen ? <X size={30} strokeWidth={2.4} /> : <Plus size={32} strokeWidth={2.4} />}</button></div><div className="nav-group">{items.slice(2).map((item) => <NavItem key={item.id} item={item} active={active === item.id} onClick={() => onNavigate(item.id)} />)}</div></div></nav></>
}
